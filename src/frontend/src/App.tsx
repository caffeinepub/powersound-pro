...
const EQ_FREQUENCIES = [
  60, 125, 250, 500, 1000, 2000, 4000, 8000, 16000, 32000,
];
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          background:
            "linear-gradient(225deg, oklch(0.12 0.06 230) 0%, oklch(0.08 0.04 220) 60%, oklch(0.06 0.02 210) 100%)",
          border: "1px solid oklch(0.35 0.14 212)",
          boxShadow:
            "inset 4px 0 24px oklch(0.72 0.28 212 / 0.15), 0 0 60px oklch(0.50 0.20 212 / 0.3)",
        }}
      >
        ...
        >
          ...
          <div className="mt-6 grid grid-cols-2 gap-2" style={{ width: "80%" }}>
            {["ENGINE 1", "ENGINE 2", "ENGINE 3", "ENGINE 4"].map((e) => (
              <div
                key={e}
                className="py-1.5 px-2 rounded text-center font-mono text-[9px]"
                style={{
                  background: "oklch(0.10 0.04 212)",
                  border: "1px solid oklch(0.28 0.10 212)",
...
export default function App() {
  ...
  const initAudioChain = useCallback(() => {
    ...
    gainNode.gain.value = volumeBooster / 100;
    gainNodeRef.current = gainNode;

    // Loud mode gain (extra boost)
    const loudGain = ctx.createGain();
    loudGain.gain.value = 1.0;
    loudGainRef.current = loudGain;

    // Chain: source → eq[0..9] → gainNode → loudGain → destination
    ...
    loudGain.connect(ctx.destination);
  }, [volumeBooster]);

  // ── Sync EQ bands to audio nodes ────────────────────────────────────────
  useEffect(() => {
  ...
  // ── Sync volume booster to gain node ────────────────────────────────────
  useEffect(() => {
    if (gainNodeRef.current) {
      // volumeBooster is 0–1700%, so gain = volumeBooster/100
      gainNodeRef.current.gain.value = Math.max(0.001, volumeBooster / 100);
    }
  }, [volumeBooster]);

  // ── Sync loud mode + chain block ────────────────────────────────────────
  useEffect(() => {
    if (loudGainRef.current) {
      // Loud mode adds extra 3x multiplier when chain is linked
      const loudMultiplier = loudMode && chainLinked ? 3.0 : 1.0;
      loudGainRef.current.gain.value = loudMultiplier;
    }
  ...
  return (
    <>
      ...
      >
        {/* biome-ignore lint/a11y/useMediaCaption: virtual audio engine */}
        ...
        <main className="flex flex-1 overflow-hidden" style={{ minHeight: 0 }}>
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              {[0, 1, 2, 3].map((i) => (
                <motion.div
                  ...
                  animate={
                    bookOpen ? { opacity: 1, x: 0 } : { opacity: 0, x: -24 }
                  }
                  transition={{ delay: i * 0.09 + 0.3, ease: "easeOut" }}
              ...
              >
                ...
                >
                  {loudMode ? "◉ LOUD BOOSTER ACTIVE" : "◎ LOUD BOOSTER OFF"}
                </button>
              </motion.div>
          ...
          >
            <div className="p-4 flex flex-col gap-3">
              ...
              <motion.div
                initial={{ opacity: 0 }}
                animate={bookOpen ? { opacity: 1 } : { opacity: 0 }}
              ...
              >
                ...
                <div
                  ...
                  style={{ borderTop: "1px solid oklch(0.18 0.04 242)" }}
                >
                  <button
                    type="button"
                    data-ocid="eq.reset_button"
                    onClick={() => setEqBands(DEFAULT_EQ)}
                    className="font-mono text-[9px] tracking-widest px-3 py-1 rounded transition-all"
                    style={{
                      background: "oklch(0.12 0.03 242)",
                      color: "oklch(0.50 0.08 212)",
              ...
              >
                ...
                >
                  <button
                    ...
                    className="w-full py-2 rounded border font-mono text-[10px] font-bold tracking-[0.35em] flex items-center justify-center gap-2 transition-all"
                    style={{
                      ...
                      boxShadow: chainLinked
                        ? "0 0 18px oklch(0.80 0.26 55 / 0.40), inset 0 0 12px oklch(0.80 0.26 55 / 0.08)"
                        : "none",
                    }}
                  >
                  ...
                  <p
                    ...
                    style={{ color: "oklch(0.40 0.04 242)" }}
                  >
                    {chainLinked
                      ? "LOUD BOOSTER → CHAIN → OUTPUT · 3× MULTIPLIER ACTIVE"
                      : "CHAIN DISCONNECTED · STANDARD OUTPUT"}
                  </p>
                </div>
              </motion.div>
